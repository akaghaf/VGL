...
sampleinp=tkinter.filedialog.askopenfilename(title= "Please select your samples.txt file:")

def fbgbfileparsing(filefbgb):

    print("Reading FreeBayes Genbank file...")
    samplesfile = sampleinp
    with open(samplesfile) as fp:
        fp = fp.read()
...
if __name__  == '__main__':
    filefbgb =tkinter.filedialog.askopenfilename(title= "Please select the Freebayes Genbank VCF:")
    filefbrs =tkinter.filedialog.askopenfilename(title= "Please select the Freebayes Refseq VCF:")
    filegagb=tkinter.filedialog.askopenfilename(title= "Please select the GATK Genbank VCF:")
    filegars=tkinter.filedialog.askopenfilename(title= "Please select the GATK Refseq VCF:")
    filestgb=tkinter.filedialog.askopenfilename(title= "Please select the Samtools Genbank VCF:")
    filestrs=tkinter.filedialog.askopenfilename(title= "Please select the Samtools Refseq VCF:")
...
#     # df = df[(df['GATK Reference Allele Frequency'] != '1.0')]
#     # df = df[(df['Samtools Reference Allele Frequency'] != '1.0')]

    
#     filteredfbgbanns = df['Freebayes GB Ann'].values.tolist() 
#     filteredgagbanns = df['GATK GB Ann'].values.tolist() 
#     filteredstgbanns = df['Samtools GB Ann'].values.tolist()
#     filteredfbrsanns = df['Freebayes RS Ann'].values.tolist()
#     filteredgbrsanns = df['GATK RS Ann'].values.tolist() 
#     filteredstrsanns = df['Samtools RS Ann'].values.tolist() 
#     Allvariants = df['Chromosome Location'].values.tolist()

#     HighVariants, ModerateVariants, LowVariants, ModifierVariants = [], [], [], []

#     for x in range(0,len(filteredfbgbanns)):
#         #filteredannstring = ""
#         filteredannstring = filteredfbgbanns[x] + filteredgagbanns[x] + filteredstgbanns[x] + filteredfbrsanns[x] + filteredgbrsanns[x] + filteredstrsanns[x]
#         strsmod = ''
#         if "HIGH" in filteredannstring:
#             strsmod += "HIGH, "
#             HighVariants.append(Allvariants[x])
#         if "MODERATE" in EffectPredictionString:
#             strsmod += "MODERATE, "
...
# Start debugging pie charts here!
# Pie Chart to debug (too many modifiers)

    # print("Calculating effect level percentages and absolute values to create pie charts...")
    # TemporaryPredictionString = ""
    # for item in Predictedeffectlevel:
    #     TemporaryPredictionString += item + " "
    # TemporaryPredictionString = TemporaryPredictionString.replace("  ", " ").replace(",", "")
    # SplitPredictionString = TemporaryPredictionString.split(" ")
    # while ("" in SplitPredictionString):
    #     SplitPredictionString.remove("")
    # modifier = 0
    # low = 0
    # high = 0
    # moderate = 0
    # for item in SplitPredictionString:
    #     if item == "LOW":
    #         low += 1
    #     elif item == "MODIFIER":
    #         modifier += 1
    ...
    # data = [modifier, low, moderate, high]
    ...
    #                                 # Adding legend
    # ax.legend(wedges, levels,
    #         title ="Levels",
    #         loc ="center left",
    #         bbox_to_anchor =(1, 0, 0.5, 1))
    # plt.setp(autotexts, size = 10, weight ="bold")
    # ax.set_title("Occurence of Levels")
    # ExplodeChartName = (str(now)+"explodechart.jpg").replace("/","_")
    # plt.savefig(ExplodeChartName, dpi = 300)
    print("Done!")
